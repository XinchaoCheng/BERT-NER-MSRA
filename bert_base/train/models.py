...
__all__ = ['InputExample', 'InputFeatures', 'decode_labels', 'create_model', 'create_model_with_softmax', 'convert_id_str',
...
def create_model_with_softmax(bert_config, is_training, input_ids, input_mask,
                 segment_ids, labels, num_labels, use_one_hot_embeddings,
                 dropout_rate=1.0, lstm_size=1, cell='lstm', num_layers=1):
    """
    创建X模型
    ...
    max_seq_length = embedding.shape[1].value
    if is_training:
        embedding = tf.layers.dropout(embedding,0.1,training=is_training)
    output = tf.layers.dense(inputs=embedding, units=num_labels)
    logits = tf.reshape(output,[-1,max_seq_length,num_labels])
