...
from bert_base.train.models import create_model_with_softmax, InputFeatures, InputExample
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        total_loss, logits, pred_ids = create_model_with_softmax(
        ...
        #print(logits)
        #tf.logging.info("**** Freeze Graph Start ****")
        #tf_config = tf.ConfigProto()
        #output_tensor_names = [logits.name,pred_ids.name]
        ##output_tensor_names = [total_loss, logits, pred_ids]
        #output_node_names = [i.split(":")[0] for i in output_tensor_names]
        #with tf.Session(config=tf_config) as tf_sess:
        #    tf_sess.run(tf.global_variables_initializer())
        #    ckpt_state = tf.train.get_checkpoint_state("/home/share/RMW/BERT-Softmax-NER/output")
        #    saver = tf.train.Saver()
        #    saver.restore(tf_sess, ckpt_state.model_checkpoint_path)
        #    frozen_graph = tf.graph_util.convert_variables_to_constants(tf_sess,
        #        tf_sess.graph.as_graph_def(), output_node_names)

        #importer.import_graph_def(frozen_graph,
        #                        {input_ids.name: input_ids,
        #                        input_mask.name: input_mask,
        #                        segment_ids.name: segment_ids,
        #                        label_ids.name: label_ids},
        #                        name="freeze")
        #logits, pred_ids = \
        #    (tf.get_default_graph().get_tensor_by_name("freeze/" + i) for i in output_tensor_names)
        #tf.logging.info("**** Freeze Graph Done ****")
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, pred_ids):
                ...
                return {
                    "eval_loss": tf.metrics.mean_squared_error(labels=label_ids, predictions=predictions),
                }
        ...
        def result_to_pair(writer):
            for predict_line, prediction in zip(predict_examples, result):
                ...
                if len(line_token) != len(label_token):
                    ...
                    break
                #print("XXXXXXXXXXXXXXXXXXXX",prediction.shape)
